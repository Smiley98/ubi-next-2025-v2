...
// s3d math extensions begin
//-------------------------------------------------------------------------------

inline Vector3 MatrixCol0(Matrix m)
{
    Vector3 v = { m.m0, m.m1, m.m2 };
    return v;
}

inline Vector3 MatrixCol1(Matrix m)
{
    Vector3 v = { m.m4, m.m5, m.m6 };
    return v;
}

inline Vector3 MatrixCol2(Matrix m)
{
    Vector3 v = { m.m8, m.m9, m.m10 };
    return v;
}

inline Vector3 MatrixCol3(Matrix m)
{
...
inline Matrix MatrixScale(Vector3 v)
{
...
inline Matrix MatrixNormal(Matrix m)
{
    Matrix n = m;
    n.m12 = n.m13 = n.m14 = 0.0f;
    n = MatrixTranspose(MatrixInvert(n));
    return n;
}

// Applies the perspective divide to v
inline Vector3 MatrixPerspectiveDivide(Matrix mvp, Vector3 v)
{
    Vector4 clip;
    clip.x = v.x;
    clip.y = v.y;
...
// Convert vector3 to array of 3 floats
inline float3 Vector3ToFloat3(Vector3 v)
{
    float3 buffer = { 0 };
...
// Convert upper 3x3 part of matrix to array of 9 floats
inline float9 MatrixToFloat9(Matrix mat)
{
    // Transposes matrix from row-major to column-major so it works with OpenGL on the GPU
    float9 result = { 0 };
...
// Convert matrix to array of 16 floats
inline float16 MatrixToFloat16(Matrix mat)
{
    // Transposes matrix from row-major to column-major so it works with OpenGL on the GPU
    float16 result = { 0 };
